...
#define DYN_THR_PID_CHANNEL THROTTLE
	...
	if (rcData[DYN_THR_PID_CHANNEL] > 1500) { // breakpoint is fix: 1500
		if (rcData[DYN_THR_PID_CHANNEL] < 2000) {
			prop2 -= ((uint16_t) conf.dynThrPID
					* (rcData[DYN_THR_PID_CHANNEL] - 1500) >> 9); //  /512 instead of /500
		} else {
	...
	rcCommand[THROTTLE] =
			lookupThrottleRC[tmp2]
					+ (tmp - tmp2 * 256)
							* (lookupThrottleRC[tmp2 + 1]
									- lookupThrottleRC[tmp2]) / 256; // [0;2559] -> expo -> [conf.minthrottle;MAXTHROTTLE]
#if defined(HEADFREE)
...
void loop() {
	...
	if ((int16_t) (currentTime - rcTime) > 0) { // 50Hz
		...
		for (i = 0; i < 4; i++)
			auxState |= (rcData[AUX1 + i] < 1300) << (3 * i)
					| (1300 < rcData[AUX1 + i] && rcData[AUX1 + i] < 1700)
							<< (3 * i + 1)
	...
	if (f.BARO_MODE) {
		...
		if ((abs(rcCommand[THROTTLE] - initialThrottleHold)
				> ALT_HOLD_THROTTLE_NEUTRAL_ZONE) && !f.THROTTLE_IGNORED) {
...
#define GYRO_I_MAX 250
	...
	ITerm = constrain((int16_t )(errorGyroI_YAW >> 13), -GYRO_I_MAX,
			+GYRO_I_MAX);
